...
export default function HomePage() {
  ...
  useEffect(() => {
    const handleReactionUpdate = (data: { postId: string; action: string; totalReactions?: number }) => {
      setPosts(prev => prev.map(p => {
        if (p.id === data.postId) {
          return {
            ...p,
            isLiked: data.action === 'created' ? true : data.action === 'removed' ? false : p.isLiked,
            // ✅ Dùng totalReactions thực tế từ backend để tránh double-count và giữ đồng bộ giữa các thiết bị
            totalReactions: data.totalReactions ?? p.totalReactions,
          };
        }
        return p;
      }));
                ...
                <article key={post.id} className="bg-surface rounded-[8px] overflow-hidden cursor-pointer" onClick={(e) => {
                  ...
                  {post.images && post.images.length > 0 && imageErrorPostId !== post.id ? (
                    <div className="relative bg-black">
                      ...
                      {post.images.length > 1 && (
                        <>
                          <button
                            onClick={(e) => { e.stopPropagation(); handleImageNav(post.id, 'prev', post.images.length); }}
                          ...
                          <button
                            onClick={(e) => { e.stopPropagation(); handleImageNav(post.id, 'next', post.images.length); }}
                  ...
                  <div className="p-3">
                    <div className="flex items-center gap-4">
                      <button
                        onClick={(e) => { e.stopPropagation(); handleReact(post.id); }}
                      ...
                      <button
                        onClick={(e) => { e.stopPropagation(); router.push(`/post-detail?postId=${post.id}`); }}
            ...
            {user && (
              <>
                ...
                <div className="flex items-center gap-3 mb-4">
                  <div
                    className="w-10 h-10 rounded-full border border-border-gray bg-surface-elevated overflow-hidden flex-shrink-0 cursor-pointer"
                    onClick={() => router.push("/profile")}
                  >
                    {user.avatar ? (
                      <img src={getFileUrl(user.avatar) || ""} alt="avatar" className="w-full h-full object-cover" />
                    ) : (
                      <svg className="w-5 h-5 text-text-secondary m-auto mt-2.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                      </svg>
                    )}
                  </div>
                  <div className="flex flex-col flex-1 min-w-0 cursor-pointer" onClick={() => router.push("/profile")}>
